[id="assembly-gs-auto-dev_{context}"]

= Getting started as an automation developer

As an automation developer, you can use {PlatformNameShort} to implement your organization's automation strategy. 
{PlatformNameShort} can help you write, test, and share automation content, and download and use Red Hat certified collections. 
This guide will walk you through the basic steps to get set up as an automation developer on {PlatformNameShort}. 
To get started as an automation developer, learn how to: 

* Set up your development environment
* Use playbooks and rulebooks
* Create roles
* Find existing automation content and publish your own custom content
* Use execution environments and decision environments
* Create a project
* Create and run rulebook activations for {EDAName}

//Add Aine's devtools doc:
//include::
include::platform/con-gs-create-automation-content.adoc[leveloffset=+1]
include::platform/con-gs-define-events-rulebooks.adoc[leveloffset=+1]
include::platform/con-gs-ansible-roles.adoc[leveloffset=+1]
include::platform/con-gs-learn-about-collections.adoc[leveloffset=+1]
include::platform/proc-gs-publish-to-a-collection.adoc[leveloffset=+1]
include::platform/proc-gs-upload-collection.adoc[leveloffset=+2]
include::platform/con-gs-execution-env.adoc[leveloffset=+1]
include::platform/proc-gs-use-base-execution-env.adoc[leveloffset=+2]
include::platform/proc-gs-add-ee-to-job-template.adoc[leveloffset=+2]
include::platform/con-gs-build-decision-env.adoc[leveloffset=+1]
include::eda/proc-eda-set-up-new-decision-environment.adoc[leveloffset=+2]
include::eda/proc-controller-adding-a-project.adoc[leveloffset=+1]
include::eda/eda-set-up-new-project.adoc[leveloffset=+1]
include::eda/con-gs-about-inv.adoc[leveloffset=+1]
include::eda/proc-controller-create-inventory.adoc[leveloffset=+2]
include::assembly-controller-job-templates.adoc[leveloffset=+1]
include::platform/con-gs-rulebook-activations.adoc[leveloffset=+1]
//include::platform/proc-eda-set-up-rulebook-activation.adoc[leveloffset=+2]
//include::platform/con-eda-rulebook-activation-list-view.adoc[leveloffset=+3]
//include::platform/proc-eda-enable-rulebook-activations.adoc[leveloffset=+2]
//include::platform/proc-eda-restart-rulebook-activations.adoc[leveloffset=+2]
//include::platform/proc-eda-delete-rulebook-activations.adoc[leveloffset=+2]
//include::platform/proc-eda-activate-webhook.adoc[leveloffset=+2]